...
import com.example.back.domain.Review;
import com.example.back.domain.ReviewLike;
...
import com.example.back.domain.ReviewImage;
...
public class ReviewService {
	...
	public Review getReview(Integer id, Integer userId) {
		Review review = reviewRepository.findById(id).orElseThrow(() -> new ReviewNotFoundException("找不到評價 ID: " + id));
	...
	public Review replyToReview(Integer id, String replyText) {
		...
		List<ReviewReport> replyReports = reviewReportRepository.findByReviewIdAndReportTarget(id, "reply");
		boolean hasActiveReplyReport = replyReports.stream().anyMatch(report -> "pending".equals(report.getStatus()));
	 ...
	 * 搜索評價
	 */
	public Page<Review> searchReviews(String keyword, Integer campSiteId, Integer minRating, Integer userId, int page,
			int size, String sortBy, String direction, Boolean includeReported) {
...
// 原來的行為：只顯示可見的評價
				reviews = reviewRepository.findByOverallRatingGreaterThanEqualAndReviewIsVisibleTrue(minRating,
						pageable);
			}
		} else {
// 使用 JPA 內建方法
...
// 附加舉報狀態
				List<ReviewReport> reviewReports = reviewReportRepository.findByReviewIdAndReportTarget(review.getId(),
						"review");
				List<ReviewReport> replyReports = reviewReportRepository.findByReviewIdAndReportTarget(review.getId(),
						"reply");

				boolean hasActiveReviewReport = reviewReports.stream().anyMatch(
						report -> "pending".equals(report.getStatus()) || "approved".equals(report.getStatus()));

				boolean hasActiveReplyReport = replyReports.stream().anyMatch(
						report -> "pending".equals(report.getStatus()) || "approved".equals(report.getStatus()));
...
// 獲取並設置圖片URL
				...
				List<String> imageUrls = images.stream().map(ReviewImage::getImageUrl).collect(Collectors.toList());
